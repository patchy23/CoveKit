...
- [ ] AR01 / AR02：收敛 AGENTS / docs/03 / docs/05 与验证矩阵，同批同步两个工程 skill（源位置待确认）；重做规范检查器及依赖守卫，保留两个 Python 薄入口并在 CI 显式运行 source_rules。
...
- [ ] AR07：API 持久化归入 HTTP/WS，保留 api_* / api.db 兼容，统一编译期装配。
