...
- [x] 2026-09-12：完成现有框架静态审查与详细任务书（23 条发现、13 项必修任务、3 项可选扩展），供后续 agent 执行；尚未实施代码整改或完成运行时验证。
...
P2 扩展 T14 / T15 / T16（工具意图、可移植备份、资源预算）仅为建议，待明确选择后进入执行范围。后续执行前先核对任务书分析基线与当前代码，避免重复修复。
