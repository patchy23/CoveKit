...
it('分页保持百条上限，筛选切换回第一页，全部连接展示远端', async () => {
  ...
  const scroll = page.get('[data-scroll-axis="vertical"]')
  expect(page.findAll('.ui-scroll-area')).toHaveLength(1)
  ...
  page.findAll('thead th').forEach((cell, index) => {
    vi.spyOn(cell.element, 'getBoundingClientRect').mockReturnValue({
      width: index === 5 ? 288 : 100,
    } as DOMRect)
  })
  ...
  const resizedWidth = (header.element as HTMLElement).style.width
  expect(resizedWidth).toContain('calc(')
  expect(scroll.classes()).toContain('overflow-x-hidden')
  expect(page.get('table').classes()).not.toContain('min-w-[1000px]')
  ...
  expect(page.get('[data-scroll-axis="vertical"]').element.scrollTop).toBe(0)
  expect((page.get('thead th').element as HTMLElement).style.width).toBe(resizedWidth)
