...
/**
 * TopBar · 顶栏（分类标题 + 计数副标题 + 搜索框 + 视图切换 + 添加按钮）
 */
...
const title = computed(() => titleMap[ui.activeCategory] ?? "全部工具");
const count = computed(() => tools.filtered.length);
</script>
...
<template>
  ...
  >
    <div>
      <h1 class="text-[17px] font-bold tracking-[-0.02em] dark:text-primary-dark">{{ title }}</h1>
      <p class="mt-[1px] text-[12px] text-text-muted dark:text-text-muted-dark">
        共 {{ count }} 个工具 · 点击卡片即可使用
      </p>
    </div>
    <div class="flex-1" />
    <div
