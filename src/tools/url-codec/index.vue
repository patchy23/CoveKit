...
<template>
  <div class="flex flex-col gap-[12px]">
    <div>
      <label class="mb-[6px] block text-[12px] font-semibold text-secondary">输入内容</label>
      <textarea
        v-model="input"
        rows="6"
        spellcheck="false"
    ...
    <p v-if="!errorMsg && input && !hasEncodedFragment(input)" class="text-[12px] text-text-muted">
      提示：当前输入不含 %xx 片段，解码前请确认内容已编码
    </p>
    <div>
      <div class="mb-[6px] flex items-center justify-between">
        <label class="text-[12px] font-semibold text-secondary">输出</label>
        <button
          v-if="output"
          class="rounded-md px-[10px] py-[4px] text-[12px] font-medium text-secondary transition-colors hover:bg-border hover:text-primary dark:text-secondary-dark dark:hover:bg-border-dark dark:hover:text-primary-dark"
