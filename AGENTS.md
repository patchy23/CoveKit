...
1. **公共编辑器组件**（CodeMirror 6 基座，22 项功能、四批约 3.5 天，**最高优先级**：所有工具都要引用它，且要一次性替换现有 10 处不成熟实现）→ 细则 `docs/tasks/2026-09-11-编辑器组件任务书.md`
2. **SSH 工具扩展四项**（终端搜索 / 右键复制当前行 / 系统信息与磁盘明细 / 会话日志），排在编辑器之后 → 细则 `docs/plugins/ssh/2026-09-11-扩展任务书.md`
3. **框架存储目录配置**（可配置数据/日志/缓存根目录 + 迁移），**待用户拍板是否本轮做**，SSH 会话日志依赖它 → 细则 `docs/tasks/2026-09-11-存储目录配置任务书.md`
