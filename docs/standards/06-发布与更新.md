...
新建凭证使用新 keyring service；只有新 service 的同空间条目明确不存在才只读回落到旧 service。拒绝访问、损坏等错误不当作缺省处理；读取兼容不重生成密钥、不改动旧条目。`.pbdata` 容器魔数、加密 AAD 和历史暂存目录前缀属于持久化协议，保持兼容；默认导出文件名前缀使用 `covekit-`。
