...
**实现状态（2026-09-21 核对）**：已接入 `log` 与 `tauri-plugin-log 2.9.2`，主实例 setup 配置 stderr、系统应用日志目录、级别和轮转；前端重要异常经官方 JS 接口写入同一输出。框架及当前 10 个后端 owner 已补关键操作与清理日志，任意错误正文不直接落盘。Windows/macOS 发布版文件输出、权限故障、轮转和退出刷新仍待平台集成验证；验证入口与执行边界见[验证矩阵](../guides/02-验证矩阵.md)，库配置参考 [Tauri 官方日志文档](https://v2.tauri.app/plugin/logging/)。

**库行为边界**：当前使用 `KeepSome(4)` 保留 4 个归档及当前文件。该版本同秒归档重名时会生成 `.bak`，其保留清理不计入普通归档，因此极端连续轮转下尚不能保证目录严格只有 5 个文件；此为未解决的上游限制，不自行添加第二套清理器。文件大小为轮转阈值，单条超大记录仍可超限。运行期写入失败沿日志库行为处理，业务调用成功不代表日志持久化成功；不承诺断电或强杀时最后一条已落盘。
...
- 合入门槛与命令只见[验证矩阵](../guides/02-验证矩阵.md)，本节描述规则覆盖，不重复执行同一扫描。
- 字段文档扫描只提示：AST 无法判断字段是否语义显然，不因缺少逐字段注释阻断提交。单位、空值、凭证与兼容含义仍需开发者说明；模块和公开 API 的职责注释继续检查。

## 10. 提交前检查（不做编译期强约束，2026-09-05 用户决策）

命令和执行平台只见[验证矩阵](../guides/02-验证矩阵.md)。`check_rust_rules.py` 与 `check_docs.py` 都会调用 Cargo，不能作为绕过本地禁止 Cargo 的“轻量脚本”。

**实现位置（AR02 起）**：两个 Python 脚本只是薄 wrapper，真正的检查在 `src-tauri/tests/source_rules/`（syn 2 AST + 夹具，约定入口 `scan_rust_rules` 与 `scan_docs`），维护者显式执行 `cargo test --test source_rules`。wrapper 用 `--exact` 精确过滤并**确认真的执行了 1 个测试**——过滤器写错导致 0 个测试不得算通过。
