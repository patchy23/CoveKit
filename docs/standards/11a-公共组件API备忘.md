...
- **UiHoverDetails 资源详情浮层**：`label` 为可访问名称，`trigger` 插槽放读数，默认插槽获得 `close` 与 `pinned`。悬停 400ms 展开，移入内容可停留；点击保持、再次点击或外部交互关闭，Esc 关闭并回到触发按钮，键盘激活将焦点移入详情。内容可承载可选择数据与操作，不能用它替代简短 Tooltip。复用 Reka 非模态定位与公共浮层样式，不依赖业务数据。
...
- **正文选择与复制**：全局默认禁止选择操作文字；`UiAlert` 正文、`UiField` 错误/说明、`UiConfirmDialog` 消息/错误、`UiModal` 描述、日志/树/列表/编辑器的错误正文默认可选。`UiEmptyState selectable` 用于失败详情；普通空状态不开放。业务插槽里的路径、地址、ID 等使用统一 `select-text`，只标记数据区域；不要给整个工具或弹窗开放选择。按钮、页签、菜单、树节点、拖动标题栏和 Tooltip 保持不可选。剪贴板调用由平台/反馈层负责，不进入基础 UI。

- **Toast 消息详情**：普通提示保持短时消失，悬停或键盘聚焦期间暂停计时，离开后重新计时；“详情”固定打开时的完整消息，可选择或复制，不受新提示覆盖。该入口不保存历史；持续的业务错误仍优先显示在页面正文。实验室“文本选择与错误复制”展示选择边界与消息详情。

- **UiFloatingWindow 非模态窗口**：父元素必须 `relative isolate` 且有确定高度；窗口限制在该工作区内，不 Teleport，不锁定背景，不替代用于表单和确认的 `UiModal`。必填 `title`，可选 `width`/`height`（默认 820/520 像素）、`activation`（递增以激活已有窗口）；`close` 由宿主卸载实例。支持标题栏拖动、右下角缩放及方向键调整尺寸、最大化和还原；点击或聚焦置顶，宿主变小会约束窗口边界。业务管理窗口集合、目标去重和关闭资源释放。

- **UiLogViewer 日志快照**：输入 `content` 完整文本快照及可选 `loading`、`error`。内置暂停显示、自动跟随、复制选中/全部、刷新和行数选择（100/300/500/1000/2000，默认 300）。事件 `limit-change(number)`、`refresh()`、`copy(text)`，宿主负责请求和平台剪贴板反馈。暂停固定显示文本、选区和滚动位置，继续接收最新尾部快照；恢复直接显示最新内容，不积累或回放历史。暂停期间调整行数，冻结内容保持到恢复；复制全部仅复制当前显示快照。用于纯文本日志，代码/配置仍使用 `UiCodeEditor`。组件实验室“日志与浮动窗口”演示多窗口与后台更新。
...
- **右键菜单更新不先卸载**：公共 UiContextMenu 不在右键按下时关闭；业务 contextmenu 已处理时保留当前实例更新内容，新公共菜单挂载才接管并关闭旧实例。未处理的外部右键、普通外部左键仍关闭；捕获监听兼容业务 stopPropagation，窗口内公共菜单保持唯一。焦点恢复使用 preventScroll，避免带动页面滚动。
...
- **UiTreeSelect**：树形单选下拉，复用 UiTree 与 Reka Popover。props：`modelValue: string`、`options: TreeSelectOption[]`、`size / disabled / label / placeholder`；节点为 `{ value, label, disabled?, children? }`，value 全树唯一，允许空字符串代表业务根节点。搜索保留命中节点的祖先，回显完整标签路径，打开时展开选中节点的祖先；搜索框按 ↓ 进入树，树内方向键导航、Enter 选择，Esc 仅收起下拉。选中发出 `update:modelValue`。业务负责排除不可选目录，不在公共组件中加入分组或 IPC 逻辑。
...
- **Vue 模板内联事件处理器不支持多语句**（2026-09-05 vite 编译报错白屏实测；2026-09-09 在 UiConfirmDialog `@confirm="emit(...); x = false; emit('close')"` 又踩一次——**「确认后连环动作」是高发形态**，写到弹窗 confirm 处理器时就要警觉）：分号/换行多语句直接被 vue compiler 拒绝（vite-error-overlay 报 Error parsing JavaScript expression）。多步操作一律抽成方法（如 `onConfirmed()`），模板只留单调用。
