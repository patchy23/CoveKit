...
**实现状态（2026-09-21 核对）**：已接入 `log` 与 `tauri-plugin-log 2.9.2`，主实例 setup 配置 stderr、系统应用日志目录、级别和轮转；前端重要异常经官方 JS 接口写入同一输出。框架及当前 10 个后端 owner 已补关键操作与清理日志，任意错误正文不直接落盘。Windows/macOS 发布版文件输出、权限故障、轮转和退出刷新仍待平台集成验证；验证入口与执行边界见[20](20-验证矩阵.md)，库配置参考 [Tauri 官方日志文档](https://v2.tauri.app/plugin/logging/)。
