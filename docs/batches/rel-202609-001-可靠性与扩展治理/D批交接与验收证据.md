...
- T11 的真机走查于 2026-09-14 部分取证：诊断报告已在真实 WebView 复制到剪贴板（并据此修掉
  权限提示缺陷）；当时没取证的「错误清单在真实崩溃路径下的留档」已于 2026-09-16 在开发构建补齐（见下
  「T11 补取证」）；真实更新下载/安装按用户 2026-09-16 裁决标 ⏸（正式版仍是占位更新公钥，等发布通道与签名密钥就位后再验）。
...
### T11 补取证（2026-09-16，开发构建）

仪器：无插桩——直接用应用自带的全局钩子（`core/diagnostics/errors.ts` 的 `installErrorCollectors`）
与设置页诊断卡片，读数经 CDP 取回。

| 分支 | 触发方式 | 记录结果 |
| --- | --- | --- |
| `window.error` | 两次真实未捕获脚本异常（页内 `setTimeout(() => { throw … })`） | `ui.uncaught_error × 2`：同 `code + message` 合并计数生效，detail 带堆栈 |
| `unhandledrejection` | 真实 `ssh_file_list` 命令传不存在的连接 id，返回的 Promise 无人接管 | `ui.unhandled_rejection × 1`，message 是后端真实错误文本「连接不存在或已断开」 |
| Vue `errorHandler` | 取容器上的真实钩子 `#app.__vue_app__.config.errorHandler(err, null, 'render')` | `ui.render_failed`，source `vue:render` |

诚实标注：前两条是真实未捕获异常（脚本发起、后端真实拒绝）；第三条走的是应用真实注册的钩子与后继留档链路，
但**触发源是脚本构造的异常**——开发构建下不改源码造不出自然渲染崩溃。

- 界面：诊断卡片按 `[code × count] message` 列出（最多 5 条），有条目时才出现「清空记录」按钮；
  离开设置回工具库再进来，三条仍在（会话内保持）。
- 报告：点「复制诊断信息」出现「已复制到剪贴板」，回读剪贴板正文含应用版本 / Tauri 版本 / 平台 /
  存储根 / 系统密钥库保护 / 保护域 / 活跃任务数，错误码行与「最近错误」列表只带 code 与 message；
  **不含命令 payload**（`ssh_file_list` 的 `pb-walkthrough-missing`、`/tmp` 均不出现）。
- 有界性（真实运行下复核，不只单测）：原有 3 条 + 连发 55 条不同消息 → 清单恒为 50 条，
  最旧的 5 条与原有 3 条被挤掉。
- 清空：点「清空记录」→ 清单 0 条、卡片回到「暂无错误记录。」、清空按钮消失。
- 口径（非缺陷）：清单是**会话级内存**，页面重载后为 0 条——`errors.ts` 文件头已写明
  「不写日志文件、不发起网络请求，内容只在本进程内，直到用户主动复制」。
