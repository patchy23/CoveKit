...
- 隐藏再显示后连接仍正常：需要真实 SSH 目标与凭证，本机无可用凭证（仍未取证）。
- 退出时长任务可取消：**2026-09-16 部分取证 + 口径待裁决**——框架长任务只有启动维护阶段的存储迁移，
  交互期不会出现，字面场景无法构造；改用插件级运行任务（frpc）走真实退出协商，见下。
- macOS 快捷键冲突、真实更新下载与安装：非本机平台 / 占位公钥环境（维持 ⏸）。
...
- 待裁决口径：`framework/tasks.rs` 的长任务当前只有存储迁移，且只跑在启动维护阶段（窗口尚不可交互），
  所以「退出时框架长任务可取消」按字面无法构造；本次取证的是插件级运行任务，走的是同一个
  `compose_decision` + `surface_veto` + 弹窗路径。要么把该验收项改写成「退出时若有任务在跑，可拒绝退出
  并取消」，要么产品上把长任务改成启动后异步执行（那属于新实现，不在本批次范围）。
