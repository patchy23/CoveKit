...
- 日志必须带级别、脱敏（同 §5）并有界保存；高频路径（轮询/逐行输出）不打日志，或只打降级后的摘要。级别体系、落盘位置与轮转由可靠性 T11 统一落地，本规范先固定「分离、脱敏、有界」三条底线
...
- 合入门槛按风险分层（细则 docs/03 §5）：相关测试 + `cargo fmt` + `clippy -- -D warnings`；发布与跨层合入再跑全量 `cargo test` 与两个检查器
