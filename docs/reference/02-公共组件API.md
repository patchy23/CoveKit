...
> 本文只维护组件接口与行为；业务页面的组件使用约束由开发规范维护，不在 API 参考中另设规则。
...
- **UiFloatingWindow 非模态窗口**：父元素必须 `relative isolate` 且有确定高度；窗口限制在该工作区内，不 Teleport，不锁定背景，不替代用于表单和确认的 `UiModal`。必填 `title`，可选 `width`/`height`（默认 820/520 像素）、`activation`（递增以激活已有窗口）、`minimizable`（显示最小化入口）；`close` 由宿主卸载实例，`minimize` 由宿主保存窗口状态并提供恢复入口。支持标题栏拖动、右下角缩放及方向键调整尺寸、最大化和还原；点击或聚焦置顶，宿主变小会约束窗口边界。业务管理窗口集合、目标去重和关闭资源释放。
...
- **UiTabs 路径提示**：`title` 提供完整路径提示，溢出菜单保留该提示与 `badge`；页签数量收纳复用 `useTabsOverflow`。
...
- **模板事件处理器**：Vue 支持以分号分隔的内联多语句；多步业务操作仍应抽成具名方法，便于处理异步、错误与资源释放。出现编译错误时核对实际表达式和转义，不将一次语法错误归因于框架不支持多语句。
